...
class TestRootParser:
    ...
    def test_find_file_by_id(self):
        ...
        root_file = RootFile(header=RootHeader(version=1), blocks=[block])
    ...
    def test_find_files_by_content_key(self):
        ...
        record1 = RootRecord(file_id=100, content_key=b"\x01" * 16, name_hash=0x1111)
        record2 = RootRecord(
            file_id=200, content_key=b"\x01" * 16, name_hash=0x2222
        )  # Same content key
    ...
    def test_get_statistics(self):
        ...
        root_file = RootFile(header=RootHeader(version=1), blocks=[block1, block2])
    ...
    def test_round_trip_v1(self):
        ...
        block = RootBlock(
            num_records=1, content_flags=0x01, locale_flags=0x02, records=[record]
        )
        root_file = RootFile(header=RootHeader(version=1), blocks=[block])
    ...
    def test_round_trip_v2(self):
        ...
        block = RootBlock(
            num_records=1, content_flags=0x08, locale_flags=0x10, records=[record]
        )
...
class TestRootModels:
    ...
    def test_root_record_model(self):
        ...
        record = RootRecord(
            file_id=12345, content_key=b"\xaa" * 16, name_hash=0x1234567890ABCDEF
        )
    ...
    def test_root_block_model(self):
        ...
        block = RootBlock(
            num_records=2, content_flags=0x01, locale_flags=0x02, records=records
        )
    ...
    def test_root_file_model(self):
        ...
        block = RootBlock(
            ...
            locale_flags=0x02,
            records=[
                RootRecord(file_id=123, content_key=b"\x01" * 16, name_hash=0x1234)
            ],
        )
...
class TestRootParserHeaderErrors:
    ...
    def test_parse_via_build_version2_roundtrip(self):
        ...
        records = [
            RootRecord(file_id=100, content_key=b"\xab" * 16, name_hash=0xDEADBEEF)
        ]
        root_file = RootBuilder.create_with_records(records, version=2)
        parser = RootParser()
        binary = parser.build(root_file)
    ...
    def test_parse_via_build_version3_roundtrip(self):
        ...
        records = [
            RootRecord(file_id=50, content_key=b"\xcd" * 16, name_hash=0xBEEFCAFE)
        ]
        root_file = RootBuilder.create_with_records(records, version=3)
        parser = RootParser()
        binary = parser.build(root_file)
