...
def mock_file_operations():
    ...
    class FileOperationMocks:
        ...
        def patch_save_file(self):
            ...
            return patch(
                "cascette_tools.commands.cdn._save_file", side_effect=mock_save_file
            )
...
def mock_tempfile_operations():
    ...
    class TempfileMocks:
        ...
        def mock_named_temporary_file(self, mode="w+b", delete=True, **kwargs):
            ...
            mock_file.write.side_effect = lambda data: self.temp_files.update(
                {temp_name: data}
            )
    ...
    with patch(
        "tempfile.NamedTemporaryFile", side_effect=mocks.mock_named_temporary_file
    ):
        with patch(
            "tempfile.TemporaryDirectory", side_effect=mocks.mock_temporary_directory
        ):
...
def mock_batch_file_operations():
    ...
    class BatchFileMocks:
        ...
        def mock_glob(self, path, pattern):
            ...
            return [
                Path(f)
                for f in self.directories[path_str]
                if pattern == "*" or pattern in f
            ]
...
def mock_cdn_and_file_operations(mock_file_operations):
    ...
    with patch("cascette_tools.core.cdn.CDNClient") as mock_cdn_class:
        mock_cdn = Mock()
        mock_cdn.fetch_config.side_effect = lambda h: cdn_mocks.mock_cdn_fetch(
            "fetch_config", h
        )
        mock_cdn.fetch_data.side_effect = lambda h, **kw: cdn_mocks.mock_cdn_fetch(
            "fetch_data", h
        )
        mock_cdn.fetch_patch.side_effect = lambda h, **kw: cdn_mocks.mock_cdn_fetch(
            "fetch_patch", h
        )
