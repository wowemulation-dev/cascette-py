...
class BLTEChunk(BaseModel):
    ...
    decompressed_checksum: bytes | None = Field(
        default=None,
        description="MD5 checksum of decompressed data (extended format only)",
    )
    compression_mode: CompressionMode = Field(description="Compression mode")
    data: bytes = Field(description="Chunk data")
    encryption_type: EncryptionType | None = Field(
        default=None, description="Encryption type"
    )
    encryption_key_name: bytes | None = Field(
        default=None, description="Encryption key name (8 bytes)"
    )
    encryption_iv: bytes | None = Field(
        default=None, description="Encryption IV (up to 8 bytes)"
    )
...
class BLTEParser(FormatParser[BLTEFile]):
    ...
    def _parse_chunks(self, stream: BinaryIO, header: BLTEHeader) -> list[BLTEChunk]:
        ...
        if header.is_single_chunk():
            ...
            for _ in range(header.chunk_count):
                ...
                if (
                    len(comp_size_bytes) != 4
                    or len(decomp_size_bytes) != 4
                    or len(checksum_bytes) != 16
                ):
                ...
                if is_extended:
                    ...
                    if len(decomp_checksum_bytes) != 16:
                        raise ValueError(
                            "Incomplete decompressed checksum in extended format"
                        )
            ...
            for comp_size, decomp_size, checksum, decomp_checksum in chunk_infos:
                ...
                if len(chunk_data) != comp_size:
                    raise ValueError(
                        f"Incomplete chunk data: expected {comp_size}, got {len(chunk_data)}"
                    )
    ...
    def _parse_encrypted_chunk(
        self, data: bytes, comp_size: int, decomp_size: int
    ) -> BLTEChunk:
        ...
        if len(data) < header_end + 1:
            raise ValueError(
                "Encrypted chunk header truncated (missing algorithm byte)"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Unknown encryption algorithm: {algorithm_byte:02x}"
            ) from e
    ...
    def _decompress_chunk(self, chunk: BLTEChunk) -> bytes:
        ...
        if chunk.compression_mode == CompressionMode.ENCRYPTED:
            ...
            try:
                ...
                raise ValueError(
                    f"Unknown inner compression mode: {inner_mode_byte}"
                ) from e
    ...
    def _decrypt_chunk(self, chunk: BLTEChunk) -> bytes:
        ...
        if not key:
            raise ValueError(
                f"Encryption key not found: {chunk.encryption_key_name.hex()}"
            )
    ...
    def _decompress_block_data(
        self, compression_mode: CompressionMode, data: bytes
    ) -> bytes:
    ...
    def _decompress_frame(self, data: bytes) -> bytes:
        ...
        while offset < len(data):
            ...
            try:
                ...
                raise ValueError(
                    f"Unknown frame compression type: {frame_comp_type}"
                ) from e

            # Decompress frame recursively
            decompressed = self._decompress_block_data(
                frame_compression_mode, frame_data
            )
    ...
    def build(self, obj: BLTEFile) -> bytes:
        ...
        for chunk in obj.chunks:
            ...
            if chunk.compression_mode == CompressionMode.ENCRYPTED:
                ...
                iv = chunk.encryption_iv or b"\x00" * 8
                iv_trimmed = (
                    iv.rstrip(b"\x00") or b"\x00"
                )  # keep at least 1 byte if all zero
                result.write(
                    struct.pack("B", self.ENCRYPTION_KEY_NAME_SIZE)
                )  # key_name_length = 8
...
class BLTEBuilder:
    ...
    def create_single_chunk(
        cls, data: bytes, compression: CompressionMode = CompressionMode.NONE
    ) -> BLTEFile:
    ...
    @classmethod
    def create_multi_chunk(
        cls, chunks: list[tuple[bytes, CompressionMode]]
    ) -> BLTEFile:
    ...
    @classmethod
    def create_multi_chunk_extended(
        cls, chunks: list[tuple[bytes, CompressionMode]]
    ) -> BLTEFile:
